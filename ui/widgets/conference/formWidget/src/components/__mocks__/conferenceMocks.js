const conferenceMock = {
  name:
    'Et veritatis quod laboriosam incidunt vitae tenetur numquam. Nemo qui maxime praesentium a voluptatem veritatis eligendi corrupti. Repudiandae omnis ad quae accusamus rem qui.',
  location:
    'Voluptatem cupiditate et doloremque. Qui ea voluptatem quos dolores. Repudiandae atque enim consequatur temporibus laboriosam iure. Blanditiis dolorem quo fugiat. Alias aut sapiente ut.',
};

export default conferenceMock;
